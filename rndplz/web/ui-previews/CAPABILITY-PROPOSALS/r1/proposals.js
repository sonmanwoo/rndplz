...
function scene(item) {
  return '<div class="pr-scene pr-scene--' + item.mood + '" aria-hidden="true">' +
    ...
    '<div class="pr-envelope"><div class="pr-envelope-back"></div>' +
...
export function initProposals(container) {
  ...
  function play(announce = false) {
    ...
    const visual = detail.querySelector('.pr-scene');
    const hint = detail.querySelector('.pr-motion-hint');
    if (!visual) return;
    visual.classList.remove('pr-is-playing');
    if (reducedMotion.matches) {
      hint.textContent = '모션 감소 설정에 따라 완성된 봉투만 보여 드려요.';
      if (announce) live.textContent = '모션 감소 설정에 따라 정지된 봉투를 표시했습니다.';
      return;
    }
    // Replaying only restarts decorative CSS; proposal data stays unchanged.
    void visual.offsetWidth;
    visual.classList.add('pr-is-playing');
    hint.textContent = '편지가 봉투에 담기는 모습 · 시연 모션';
    if (announce) live.textContent = '봉투 모션을 다시 보여 드립니다. 제안 상태는 바뀌지 않습니다.';
    playTimer = setTimeout(() => visual.classList.remove('pr-is-playing'), 2400);
  }
  ...
  function onClick(event) {
    ...
    if (button.dataset.prFilter) {
      ...
      if (event.detail === 0 || window.matchMedia('(max-width: 760px)').matches) {
        ...
        title.scrollIntoView({ behavior: reducedMotion.matches ? 'auto' : 'smooth', block: 'start' });
      }
  ...
  function onMotionChange() { play(false); }
